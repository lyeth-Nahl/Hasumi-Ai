...
module.exports = {
  ...
  Ayanokoji: async function ({ api, event, args, usersData }) {
    try {
      ...
      if (isNaN(numberOfImages) || numberOfImages <= 0 || numberOfImages > 5) {
        return api.sendMessage("Please specify a number between 1 and 5.", event.threadID, event.messageID);
      }
