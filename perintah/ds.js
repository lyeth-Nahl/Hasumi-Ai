...
module.exports = {
  ...
  Ayanokoji: async function({ api, event, args }) {
    if (args[0] === 'gambar') {
      ...
      api.sendMessage({
        body: `Gambar: ${kartu.nama} (${kartu.rarity})`,
        attachment: await getBuffer(kartu.gambar)
      }, event.threadID);
    } else if (args[0] === 'info') {
      ...
      if (dataStatus[event.senderID].yen >= hargaPull) {
        ...
        const kartu = dsData[Math.floor(Math.random() * dsData.length)];
        api.sendMessage({
          body: `Kamu berhasil menarik kartu! Kamu mendapatkan ${kartu.nama} (${kartu.rarity})!`,
          attachment: await getBuffer(kartu.gambar)
        }, event.threadID);
      } else {
      ...
      if (!dataStatus[event.senderID]) {
        ...
        if (cardIndex === -1) {
          ...
          await fs.promises.writeFile(statusPath, JSON.stringify(dataStatus, null, 2));
