...
module.exports = {
  ...
  Ayanokoji: async function ({ api, event, args, bhs }) {
    ...
    const idPenerima = parseInt(args[0]);
    ...
    const penerima = Object.values(statusData).find((user) => user.id === idPenerima);
    ...
    api.sendMessage(notifikasi, Object.keys(statusData).find((key) => statusData[key].id === idPenerima));
  }
};