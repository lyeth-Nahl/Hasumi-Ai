...
module.exports = {
  ...
  Ayanokoji: async function ({ api, event, args, client, __ }) {
    ...
    if (args[0] === "install") {
      ...
      api.sendMessage(`Anda yakin ingin menginstal kode baru untuk perintah ${namaCmd}? Ketik "konfirmasi" dalam waktu 50 detik untuk melanjutkan.`, event.threadID, event.messageID);

      const waktuTunggu = 50 * 1000; // 50 detik dalam milidetik
      const konfirmasi = await api.getMessage(event.threadID, event.messageID, waktuTunggu);

      if (konfirmasi && konfirmasi.body.toLowerCase() === "konfirmasi") {
