module.exports = {
  ...
  Ayanokoji: async function ({ api, event, args, client, __ }) {
    ...
    if (args[0] === "install") {
      ...
      api.sendMessage(`Anda yakin ingin menginstal kode baru untuk perintah ${namaCmd}? Ketik "konfirmasi" untuk melanjutkan.`, event.threadID, event.messageID);

      const konfirmasi = await api.getMessage(event.threadID, event.messageID);

      if (konfirmasi.body.toLowerCase() === "konfirmasi") {
