...
Masukkan tebakan (huruf atau kata), atau 'skip' untuk melewati kata ini:`, user);

api.on('message', (message) => {
  if (message.senderID === user && message.body !== '') {
...
_Nyawa: ${nyawa}_
_Hint: ${hint}_
_Skip: ${skip}_`, user); } else { // Tebakan adalah kata if (tebakan === kata[0]) { jawabanBenar++; api.sendMessage(`Kamu menang! Jawaban benar: ${jawabanBenar}`, user); if (jawabanBenar === 1) { yen += 3; } else { yen += jawabanBenar * 3; } api.sendMessage(`Kamu mendapatkan ${yen} yen!`, user); // Simpan data user if (!userData[user]) { userData[user] = {}; } userData[user].yen = yen; } else { nyawa--; api.sendMessage(`Kata salah! Nyawa: ${nyawa}`, user);
}
}
}
